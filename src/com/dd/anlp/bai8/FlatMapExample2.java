...
public class FlatMapExample2 {
    public static void main(String[] args) {
        ...
        // sum of the line 'items' total amount
        BigDecimal sumOfLineItems = orders.stream().flatMap(order -> order.getLineItems().stream()).map(line -> line.getTotal()).reduce(BigDecimal.ZERO, BigDecimal::add);

        // sum of the order's total amount
        BigDecimal sumOfOrder = orders.stream().map(order -> order.getTotal()).reduce(BigDecimal.ZERO, BigDecimal::add);
